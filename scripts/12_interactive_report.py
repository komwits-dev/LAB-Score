#!/usr/bin/env python3
"""
12_interactive_report.py — LAB-Score v1.2 Interactive HTML Report
"""
...
<title>LAB-Score v1.2 — Interactive Report</title>
...
<div id="page-about" class="page">
  ...
  <div class="g2">
    <div>
      <div class="card">
        <h2>🧫 About LAB-Score v1.2</h2>
        <p>LAB-Score v1.2 is a genome-based scoring framework for prioritizing <i>Lactobacillaceae</i> strains as probiotic or fermentation candidates. It integrates safety screening, GI survival capacity, probiotic-associated functional markers, and fermentation traits into a single interpretable composite score.</p>
...
&nbsp;&nbsp;+ 0.10 × Fermentation_score
        ...
        <div style="margin-top:6px">
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <span class="badge be">≥95 Elite</span>
            <span class="badge bh">80–94 High</span>
            <span class="badge bm">60–79 Moderate</span>
      ...
      <div class="card">
        ...
        <div class="step-card">
          ...
          <div class="step-body">
            ...
            <p>All features merged into a master matrix. LAB-Score v1.2 calculated using the weighted formula. Refined safety gate applied to assign final candidate tiers.</p>
...
<footer>LAB-Score v1.2 Interactive Report — Generated by LAB-Score Pipeline v3.5</footer>
...
function scorebar(v) {
  ...
  const col = p>=95?'#1B4332':p>=80?'#2D6A4F':p>=60?'#52B788':'#B7E4C7';
...
function renderOverview() {
  ...
  Plotly.newPlot('ch-hist',[{x:D.score_hist,type:'histogram',nbinsx:50,
    marker:{color:D.score_hist.map(v=>v>=95?'#1B4332':v>=80?'#2D6A4F':v>=60?'#52B788':'#B7E4C7'),line:{color:'#fff',width:.5}},
    hovertemplate:'Score: %{x:.1f}<br>Count: %{y}<extra></extra>'}],
    lay({shapes:[60,80,95].map((x,i)=>{return{type:'line',x0:x,x1:x,y0:0,y1:1,yref:'paper',
...
function openModal(idx){
  ...
  const scoreSec=`<div class="msec">
    ...
    ${[['LAB-Score v1.2',(r.LAB_score_v1||0).toFixed(2)],['Priority Class',r.Priority_class],
